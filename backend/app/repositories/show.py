...
class ShowRepository:
    ...
    def create_show(self, band_id: int, venue: str, show_date: date,
        ...
        if show.show_members:
            show.show_members = json.loads(show.show_members)
    ...
    def update_show(self, show_id: int, venue: Optional[str] = None,
                   ...
                   payment: Optional[Decimal] = None, piece_count: Optional[int] = None,
                   status: Optional[ShowStatus] = None, poster: Optional[str] = None,
                   description: Optional[str] = None) -> Optional[Show]:
        """Update show details"""
        show = self.db.query(Show).filter(Show.id == show_id).first()
        if show:
